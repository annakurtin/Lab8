...
```{r eval=F, include=F}
# how to install packages in development 
install.packages("devtools")
library(devtools)
devtools::install_github("glmmTMB/glmmTMB/glmmTMB")
# download broom for using with the mixed effects model 
devtools::install_github("bbolker/broom.mixed", type ="source")
# source indicates whether the packages are compiled 
install.packages("broom.mixed")
```

*The model comparison methods we learned in Lab6 don't work with mixed effects models*

*no way to dredge a mixed effects model? maybe there is*
...
# make sure there are no NAs in the columns of interest
...
```{r}
# our naive model with no random effects
...
```{r}
# use glmer with random intercept for elk id and a random coefficient for individual elk's response to predation risk 
...
summary(fr.rc)
# differences between naive logit and random effects is that the mixed effects model assumes a normal distribution in the response to the random effects
# we see that there is double the variation between elk than there is at the population of elk
# since they're scaled, we can compare them directly 
...
coef(fr.rc) # fixed effects and random effects coefficients for each intercept (each individual elk)
elk2$fr.rc.pred <- predict(fr.rc, type = "response")
hist(elk2$fr.rc.pred)
```
Note that this is the histogram of predictions to both the 0's and 1's in the dataset, not just the 0's as Avgar et al. (2017) advocate to be able to interpret the probability of use, not just selection, given the distribution of available covariates in a study area (see lab presentation, and forthcoming R code example).
...
# same as line 93 above - default argument for re.form is NA
...
elk2 <-
  elk2 %>% 
  as_tibble() %>% 
  mutate(w=if_else(used==0, 5000,1), # if the used is 0 we make a weight of 5000, lets you play around with if your conclusions are sensitive to your weighting
...
Learn about glmmTMB (generalized linear mixed model template model builder) here
The most customizeable way to build glms
?glmmTMB
...
So, if you always specify your random intercept(s) **first**, then it's easy to make sure that you don't accidentally fix the variance of a random slope. If you do screw it up, you will see it in the summary output. Always check the summary to make sure the variance of the random intercept(s) is 1,000,000 (has blown up to be infinite). This model took ~ 30 seconds to run on my CPU. `glmmTMB` is fast.

## Random intercept only
```{r intercept_only_model}
system.time(
  forage_risk_r_int <- glmmTMB(used ~ totalherb_sc + totalherb2_sc + log_risk_sc + # use log predation risk since it is very non normally distributed
                                 # herb plus herb squared is intermediate forage biomass
                               (1|elkuid), # inclulde a random intercept
                             weights = w, data=elk2, family=binomial,
                             map=list(theta=factor(NA)), # theta is variance component of random effects, we're making it not a factor and giving it a starting value of a really small value
                             start=list(theta=log(1e3)))
)

summary(forage_risk_r_int)
# estimate for totalherb2_sc: negative quadratic, meaning that they like increasing levels of forage biomass up until a certian point when it declines
```
...
system.time(
  forage_risk_slope_risk <- glmmTMB(used ~ totalherb_sc + totalherb2_sc + log_risk_sc + 
                                    (1|elkuid) + (0 + log_risk_sc|elkuid), # say that the random intercept is independent from the coefficient for slope of risk by elk id
                                  data=elk2, family=binomial, weights = w, 
                                  map=list(theta=factor(c(NA, 1))), # NA for random effects variance, 1 for random effect of predation
                                  start=list(theta=c(log(1e3), 0))) # small starting theta, no starting value
)

summary(forage_risk_slope_risk) # the predation coefficient is no longer significant
# tells you that some elk respond strongly to predation risk 
...
system.time(forage_risk_slopes_both <- glmmTMB(used ~ totalherb_sc + totalherb2_sc + log_risk_sc + 
                                  ...
                                  data=elk2, family=binomial, weights = w, 
                                  map=list(theta=factor(c(NA, 1:3))), # three random effects
...
# this just shows fixed effects variance - for random effects variance you have to use boot.mer
```
...
```{r}
# distribution of individual coefficients from first part of quadratic with the marginal plotted on it 
# this is fitting the inhomogenous points process model correctly with logistic regression 
as_tibble(rownames_to_column(coef(forage_risk_slopes_both)$cond$elkuid, "elkuid")) %>% 
...
forage_ran_coefs <- broom.mixed::tidy(forage_risk_slopes_both, effects = "ran_vals") %>%
  filter(term=="totalherb_sc") %>%  # get for just the first coefficient
...
forage_ran_coefs
#0:38:30 in the lab recording
# fixed effect + ranef = confidence interval
## this is still just with the fixed effects variance 
```
...
# this shows you the population mean and how individuals vary around it 
# useful for diagnosing whether you believe conditional vs marginal 
```
...
forage_for_predict_population <-
  tibble(elkuid = NA, # we're not making conditional predictions - not predicting for any individ elk
         ...
         log_risk_sc = 0, # set predation risk equal to zero
         w = NA) # ignore weighting
...
```{r make_fixed_preds}
# predict into the dataframe we just created (forage_for_predict_pop)
pop_pred_unfixed <-
  forage_for_predict_population %>% 
...
pop_pred_unfixed %>% 
  ...
  select(totalherb)
# we get this by finding the max of the quadratie - the derivative 
```
...
pred_CI <- function(model, newdata=NULL, alpha=0.05) {
  ...
  V <- vcov(model)$cond[-1,-1]     # get rid of the random effects 
...
pred_CI(forage_risk_slopes_both_UNFIXED, forage_for_predict_population) %>% 
  ...
  geom_line(size=1) + 
  theme_classic(base_size=15)
# 95 CI from fixed effects on model ignoring random intercept variance  and ignoring the random coefficient variance 
```
...
# Individual-level predictions 0:59:20 in recording
...
# some lines don't go all the way bc the elk never went to areas with certain forage quality
```
...
for_predict_ind %>%
  ...
  ggplot(., aes(x=totalherb, y=pred, color=elkuid)) +
    ...
    theme_classic(base_size=15) +
    coord_cartesian(xlim= c(0,200), ylim=c(0,10))
# this shows the pop average and the 95% confidence intervals 
```
...
for_predict_ind %>%
  ...
  ggplot(., aes(x=totalherb, y=pred, color=elkuid)) +
    ...
    theme_classic(base_size=15) +
    coord_cartesian(xlim= c(0,200), ylim=c(0,10))
# on the y axis here is a probability of selection 
```
...
# this gives the predicted probability of use for just the zeros
# above was the predicted probability of selection (for the 1s and 0s)
```
